import {
  ExtendedDetails,
  SeriesObject,
} from '../../../../data/modules/detailed/detailed-movie.metadata';
...
export class SidenavComponent {
  ...
  constructor(
    ...
    this.actions$
      ...
      .subscribe((data: CompletedAction) => {
        ...
        this.getCurrentSeriesObject(
          data.action.season,
          (currentSeasonObject) => {
            if (currentSeasonObject.screenLoading) {
              this.detailedMovieController.toggleLoading(
                data.action.season,
                false
              );
            }
  ...
  public toggleAccordion(opened?: boolean, season?: number): void {
    ...
    // always
    this.store.dispatch(
      new DetailedMovieActions.ToggleAccordion(season, newOpened)
    );
    ...
    this.getCurrentSeriesObject(season, (currentSeasonObject) => {
      ...
      if (newOpened) {
        if (!currentSeasonObject.data.length) {
          ...
          // fetch and set data
          this.detailedMovieController.fetchAndSetSeries(
            currentSeasonObject.movieId ?? 0,
            season
          );
        } else {
        }
  ...
  private getCurrentSeriesObject(
    season: number,
    clbck?: (val: SeriesObject) => void
  ) {
    this.series$
      .pipe(
        mergeMap((seriesArr) =>
          seriesArr.filter((ep) => (ep?.number ? ep.number === season : false))
        ),
        first()
      )
      .subscribe((obj) => {
